...
2. `lib`안에 `src` 폴더를 만들고,

   `src`폴더 안에 `blocs` `models` `resources` `ui` `values` 폴더 만들기

   (`app.dart`도 `src`파일 안으로 옮기기)
...
7. Repository 구현
   ...
   ***Provider의 네트워크 콜과 Repository는 항상 셋트다. 네트워크 콜이 일어나면 데이터 BLOC으로 전달해주기 위한 중간다리인 Repository에 전달되어야한다. 따라서 Provider에 추가되면, Repository에도 추가되어야한다.***
